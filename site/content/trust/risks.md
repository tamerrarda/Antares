...
## 17. Gaps in the evidence, as opposed to the design

- **No round has closed on the deployed vault.** What is proven today is the deployment, not the
  cycle.
- **Two live drills have recorded results and no runnable script.** The pause and upgrade drills were
  executed against testnet on 2026-08-21 with real transactions, driven by hand rather than by a
  committed script, so the procedure is not reproducible from a clean checkout.
...
- **One stated precondition in I10.** If the price feed lengthens its own tick interval mid-round,
  ...
  in-the-money buyer their payout. Exposure is one round rather than open-ended.
- **The differential corpus is red on a known open item.** The four hand-written vectors agree byte
  for byte across all four replayed sections; a generated 204-vector corpus does not, and about 87
  fail from a single categorised cause the repository records as a specification disagreement rather
  than a defect in either implementation.

All five are on [Where this actually stands](../start/status.md), which is their home.
