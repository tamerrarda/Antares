...
export interface BindingProvenance {
   ...
   * because records written before the host was measured do not carry it, and the check treats
   * their absence as the finding rather than assuming a host.
   */
...
export function checkBindings(input: BindingsCheckInput): Check[] {
  ...
  if (input.committedProvenance !== null) {
    ...
    // The hash is only comparable to a build from the same host. On a foreign host
    ...
    checks.push(
      recorded.buildHost === input.host
        ...
        : mkCheck(
            ...
            `The host is an input to this hash. Recorded ${recorded.wasmSha256} on ` +
  ...
  if (write && passed("bindings.generator_pinned") && passed("bindings.wasm_fresh")) {
    ...
    const provenance: BindingProvenance = {
      ...
      _what:
        ...
        "wasm, because the host is an input to the hash and used to be an invisible one.",
    };
